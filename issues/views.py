...
def getallissues(request):
    val = requests.get('https://api.github.com/search/issues?q=state=closed&per_page=100&sort=created&dir=asc', headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
    print val
    print dir(val)
    print type(val)
...
#     val = requests.get('https://api.github.com/search/issues?q=state:closed&since=2015-12-30T22:52:57Z&sort=created&per_page=100&order=desc&page=1', headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
    val = requests.get('https://api.github.com/search/issues?q=state:open+created:>='+yesterday.isoformat()+'&per_page=100&order=asc&page='+str(page_id), headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
    print val
    json_data = json.loads(val.text);
    print json_data["total_count"]
...
def home(request):
    yesterday = datetime.now() - timedelta(days=1)
    print yesterday.isoformat()
    val = requests.get('https://api.github.com/search/issues?q=state:open+created:>='+yesterday.isoformat()+'&per_page=100&order=asc', headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
    json_data = json.loads(val.text);
    print json_data
    week = datetime.now() - timedelta(days=7)
    val1 = requests.get('https://api.github.com/search/issues?q=state:open+created:"'+week.isoformat()+' .. '+yesterday.isoformat()+'"&per_page=100&order=asc', headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
    week_data = json.loads(val1.text);
    yesterday = datetime.now() - timedelta(days=1)
    val2 = requests.get('https://api.github.com/search/issues?q=state:open+created:<='+week.isoformat()+'&per_page=100&order=asc', headers={'Authorization': 'token 1525b4b004c6368157886f1e2e56f117b566a80f'})
